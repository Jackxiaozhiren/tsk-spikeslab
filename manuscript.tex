...
% Increase emergency stretch to reduce overfull hbox in narrow single-column
...
\begin{equation}
 ...
 \begin{cases}
  \mathcal{N}(\mathbf{0}, \tau^2 \mathbf{I}_{d+1}), & \gamma_j = 1 \;\;\text{(slab)}\\[4pt]
  \delta_{\mathbf{0}}(\boldsymbol{\beta}_j),                      & \gamma_j = 0 \;\;\text{(spike)}
 \end{cases}
 \label{eq:spikeslab}
\end{equation}
with $\gamma_j\sim\text{Bernoulli}(\pi)$, $\pi\sim\text{Beta}(a_\pi,b_\pi)$, and slab variance $\tau^2$. Setting $a_\pi=b_\pi=1$ induces a uniform prior over the number of active rules \cite{gelman2013bayesian}.
...
\emph{Step 4: Prediction intervals.} Predictive uncertainty is quantified through the posterior covariance under the selected model. The posterior precision matrix for $\boldsymbol{\beta}$ is $\mathbf{P} = \boldsymbol{\Phi}^\top\boldsymbol{\Phi}/\hat{\sigma}^2_\varepsilon + \text{diag}(\boldsymbol{\tau}^{-1})$, where $\hat{\sigma}^2_\varepsilon$ is estimated from the ridge residual variance, and $\boldsymbol{\tau}$ encodes slab variance ($\tau^2 = 10^3$) for active rules and near-zero variance for pruned rules. The predictive variance for a new input $\mathbf{x}^*$ with design row $\boldsymbol{\phi}^*$ is:
\begin{equation}
...
This analytical approximation replaces the MCMC sampling loop (each Gibbs iteration requiring $O(R^3 + nR^2)$ for conditional posterior updates) with a single $O(R \cdot n\cdot Rd)$ pass through the rules plus one $O((R(d+1))^3)$ matrix inversion, making it practical for interactive exploratory analysis. The approximation inherits two known limitations: (a) BIC-based Bayes factors are accurate in large-$n$ regimes but can mis-rank models when the effective sample size per rule is small; (b) the Gaussian predictive approximation underestimates tail uncertainty, particularly when the posterior over $\gamma_j$ is not concentrated at 0 or 1 \cite{gelman2013bayesian}. We return to these limitations when interpreting the empirical results.

%============================================================
\section{Experimental Setup}
\label{sec:experiments}

We evaluate on three UCI benchmarks with four target variables (Table~\ref{tab:datasets}). The Energy Efficiency dataset \cite{tsanas2012accurate} contains 768 simulated building configurations (8 geometric parameters). Heating (Y1) and cooling (Y2) loads are modeled as separate regression targets. The Concrete Compressive Strength dataset \cite{yeh1998modeling} contains 1030 laboratory samples (8 mix-design parameters). To test the hypothesis that sparsity becomes more advantageous in higher dimensions, we add the Facebook Metrics dataset \cite{moro2016predicting}---19 features derived from Facebook page activity, subsampled to $n=1000$ for computational tractability. Features are standardized to zero mean and unit variance before modeling; no samples contain missing values.

\begin{table}[htbp]
\centering
\caption{Dataset characteristics. All features are continuous and standardized to zero mean and unit variance before modeling. No samples contain missing values.}
\label{tab:datasets}
\begin{tabular}{lcccc}
\toprule
Characteristic & Energy Efficiency & Concrete CS & Facebook Metrics \\
\midrule
Samples ($n$) & 768 & 1030 & 1000$^*$ \\
Features ($d$) & 8 & 8 & 19 \\
Target & Heating/Cooling Load (kWh/m$^2$) & Strength (MPa) & Total Interactions \\
Data type & Simulation (Ecotect) & Laboratory & Social media \\
\bottomrule
\end{tabular}
\par\vspace{4pt}
{\footnotesize $^*$Random subsample from 5{,}875 original samples for computational tractability of the $R(d+1)$-dimensional TSK design matrix \cite{moro2016predicting}.}
\end{table}

Six methods are compared: \textbf{TSK-LS} (classical least-squares, dense baseline); \textbf{TSK-LASSO} (L$_1$-regularized, $\lambda$ via 5-fold CV); \textbf{TSK-SpikeSlab} (proposed, $\pi\sim\text{Beta}(1,1)$, $\alpha=10^{-2}$, $\tau^2=10^3$); \textbf{Bayesian-TSK} (non-sparse conjugate prior \cite{gu2018bayesian}, prior scale matched to isolate sparsity effect); \textbf{Random Forest} (300 trees, max\_depth=10); and \textbf{SVR} (RBF kernel, $C=1.0$, $\gamma=\text{scale}$). All TSK methods use $k$-means $k=5$ with Gaussian membership spread scaling factor $s=1.5$ (applied to cluster standard deviations). A fixed base random seed of 42 is used, with split-specific seeds $42 + s$ for the $s$-th split to ensure reproducibility. All experiments use 30 independent random 80/20 train/test splits.

Each dataset undergoes 30 independent random 80/20 train/test splits. Metrics are RMSE, MAE, $R^2$, active rules (PIP $>0.5$ for TSK-SpikeSlab; $|\boldsymbol{\beta}_j|_2 > 10^{-6}$ for TSK-LASSO), PICP, and MPIW. Significance is assessed via Wilcoxon signed-rank with Bonferroni correction for 15 pairwise tests ($\alpha=0.05/15=0.0033$) \cite{wilcoxon1945individual}, and global ranking via Friedman--Nemenyi across the three datasets \cite{demsar2006statistical}. Effect sizes (Cohen's $d$) are reported for key pairwise comparisons to complement inferential tests. Ablation studies vary $\pi\in\{0.1,0.3,0.5,0.7,0.9\}$ and $k\in\{3,5,7,9\}$ on Energy-Cooling (10 splits).
...
Tables~\ref{tab:main}--\ref{tab:uncertainty} report the primary results across three low-dimensional target variables. Bayesian-TSK achieves the best RMSE among fuzzy methods on all datasets. On Energy-Cooling, Bayesian-TSK (RMSE $=4.11\pm0.80$, $R^2=0.805$) substantially outperforms classical TSK-LS (RMSE $=5.60\pm4.80$, $R^2=0.408$). Random Forest yields the lowest RMSE overall, as expected for low-dimensional tabular data.
...
\subsection{Higher-Dimensional Validation}

To test whether the sparsity-accuracy trade-off improves with dimensionality, we evaluate all methods on the Facebook Metrics dataset ($d=19$, $n=1000$ subsampled; Table~\ref{tab:highdim}). At $d=19$, the relative accuracy cost of sparsity is substantially reduced: TSK-SpikeSlab achieves RMSE $=24.40$, which is 1.9$\times$ worse than TSK-LS (RMSE $=12.56$), compared with the 3.4--3.8$\times$ degradation observed at $d=8$. Notably, interval calibration also improves dramatically: TSK-SpikeSlab's PICP rises from $0.077$ (Energy-Cooling, $d=8$) to $0.695$ (Facebook, $d=19$), though it still falls short of the nominal 95\% level. Bayesian-TSK maintains well-calibrated intervals (PICP $=0.949$) at $d=19$, confirming that the conjugate Gaussian--Wishart framework's calibration advantage persists across dimensionalities. These results support the hypothesis that sparsity becomes less costly as dimensionality increases: at $d=19$, the design matrix $\boldsymbol{\Phi}$ has $R(d+1)=100$ columns versus 45 at $d=8$, and the larger parameter space provides more room for rule pruning to operate without catastrophic loss of input space coverage.
...
\begin{table}[htbp]
\centering
\caption{Performance comparison on low-dimensional benchmarks (mean $\pm$ std, 30 splits). Best in \textbf{bold}, second \underline{underlined}. Active rules: PIP$>0.5$ (TSK-SpikeSlab); $|\boldsymbol{\beta}_j|_2 > 10^{-6}$ (TSK-LASSO).}
\label{tab:main}
\footnotesize
...
\begin{table}[htbp]
\centering
\caption{Performance on Facebook Metrics ($d=19$, $n=1000$, 30 splits). Higher dimensionality reduces the relative accuracy cost of sparsity (1.9$\times$ vs 3.8$\times$ at $d=8$) and substantially improves interval calibration (PICP $=0.695$ vs $0.077$ at $d=8$).}
\label{tab:highdim}
\begin{tabular}{lccccll}
\toprule
Method & RMSE$\downarrow$ & $R^2$$\uparrow$ & Rules & PICP & MPIW \\
\midrule
TSK-LS        & 12.56$\pm$7.55 & $-$2.42 & 5.0 & -- & -- \\
TSK-LASSO     & 22.90$\pm$0.91 & $-$7.41 & 5.0 & -- & -- \\
TSK-SpikeSlab & 24.40$\pm$0.62 & $-$8.54 & 3.0 & 0.695 & 66.92 \\
Bayesian-TSK  & \textbf{9.08$\pm$1.98} & \textbf{$-$0.39} & 5.0 & 0.949 & 40.91 \\
Random Forest & \underline{3.84$\pm$0.35} & \underline{0.763} & -- & -- & -- \\
SVR           & 7.19$\pm$0.30 & 0.173 & -- & -- & -- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{results/figures/fig3_mechanism.pdf}
\caption{RMSE by posterior predictive uncertainty bin. TSK-SpikeSlab consistently outperforms TSK-LASSO on Energy-Cooling ($\Delta = 9.9$--$11.4$); TSK-LS shown as reference. Per-bin sample size $n\approx 51$ per split. On Concrete, differences are $<1$ RMSE ($<5\%$ of the scale)---below practical significance.}
\label{fig:mechanism}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{results/figures/fig4_fcm_sensitivity.pdf}
...
The Friedman test across the three datasets rejects equal performance ($\chi^2=147.2$, $p<10^{-8}$). Average ranks across datasets: Random Forest (1.00) $\prec$ SVR (2.21) $\prec$ Bayesian-TSK (2.75) $\prec$ TSK-LS (3.80) $\prec$ TSK-SpikeSlab (5.12) $\prec$ TSK-LASSO (6.00). Wilcoxon tests with Bonferroni correction confirm TSK-SpikeSlab significantly outperforms TSK-LASSO on Energy datasets ($p<0.001$, Bonferroni $\alpha=0.0033$) but not on Concrete ($p=0.792$). The RMSE differences on Energy datasets are large in magnitude: Cohen's $d$ for TSK-SpikeSlab vs.\ TSK-LASSO ranges from $-4.71$ to $-6.74$ (large effect in favor of TSK-SpikeSlab), while on Concrete the difference is negligible ($d=-0.03$). All other pairwise comparisons are significant at the corrected level with $|d| > 2$.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{results/figures/fig5_cd_diagram.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{results/figures/fig6_rules.pdf}
...
Datasets are publicly available from the UCI Machine Learning Repository: Energy Efficiency (dataset 242) \cite{tsanas2012accurate}, Concrete Compressive Strength (dataset 165) \cite{yeh1998modeling}, and Facebook Metrics \cite{moro2016predicting} (subsampled to $n=1000$ as described in Section~\ref{sec:experiments}). Additional supplementary results on the UCI Air Quality dataset (dataset 360) are available in the Supplementary Material. Code is available at \texttt{https://github.com/Jackxiaozhiren/tsk-spikeslab}. MAE values for all methods and datasets are reported in the Supplementary Material.
...
During the preparation of this work the author did not use any generative AI tools for scientific writing. All analyses, figures, and text were produced by the author.
...
The Supplementary Material includes: (1) full MAE values for all methods across Energy (Heating), Energy (Cooling), Concrete, Facebook Metrics, and Air Quality datasets; (2) extended TSK-LASSO diagnostic code with expanded $\alpha$ grid results (Fig.~S1); (3) $\tau^2$ ablation table (Table~S2); (4) results on the UCI Air Quality dataset ($n=9357$, $d=15$, dataset 360). On Air Quality, TSK-LASSO achieves the best RMSE among fuzzy methods (0.417, vs.\ TSK-SpikeSlab 0.707), demonstrating that the method's relative performance depends on dataset characteristics---specifically, on the degree of multicollinearity in the weighted TSK design matrix. TSK-LS performs poorly on Air Quality (RMSE $=2.236$, $R^2=-60.9$), suggesting that the baseline dense model fails catastrophically on this dataset due to extreme collinearity. These supplementary results complement the main text by showing the full spectrum of method behavior, from data regimes where LASSO succeeds to those where it collapses.
